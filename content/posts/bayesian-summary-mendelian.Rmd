...
Here, the idea comes from the fact that $\hat\theta$ coming from single-effect model (GWAS summary statistic) follows $\hat\theta \sim N(SRS^{-1}\theta, SRS)$ where $\theta$ is the effect of true multi-site model. Following this idea, the paper proposed a likelihood function for $\hat\theta$ given $\hat\alpha$. Specifically, $\hat\theta \sim N(SRS^{-1} \sum_{k = 1}^K \hat{\vec\alpha_k} \beta_k + \vec\gamma$. 
